...
const randomValidFirstname = faker.name.firstName();
const randomValidLastname = faker.name.lastName();
...
describe('email check', () => {
	it('knows that Tim.Kertzscher@web.de is valid Email', () => {
		expect(validateSignUp(randomValidUsername, 'Tim.Kertzscher@web.de', randomValidPassword, randomValidPassword,
			randomValidFirstname, randomValidLastname)).toBe(true);
	});
	it('knows that mail with missing @ is incorrect Email', () => {
		expect(validateSignUp(randomValidUsername, 'Tim.Kertzscher.web.de', randomValidPassword, randomValidPassword,
			randomValidFirstname, randomValidLastname)).toBe(false);
	});
	it('knows that mail with missing . is incorrect Email', () => {
		expect(validateSignUp(randomValidUsername, 'Drachenlord@webde', randomValidPassword, randomValidPassword,
			randomValidFirstname, randomValidLastname)).toBe(false);
	});
...
describe('name check', () => {
	...
	it('knows that Kertzscher is valid lastname', () => {
		expect(validateSignUp(randomValidUsername, randomValidEmail, randomValidPassword, randomValidPassword,
			randomValidFirstname, 'Kertzscher')).toBe(true);
	});
	...
	it('knows that empty string is invalid lastname', () => {
		expect(validateSignUp(randomValidUsername, randomValidEmail, randomValidPassword, randomValidPassword,
			randomValidFirstname, '')).toBe(false);
	});
});
